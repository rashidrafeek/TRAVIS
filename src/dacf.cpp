...
//	sprintf(buf,"%s_%s",((CMolecule*)g_oaMolecules[m_iFirstMol])->m_sName,((CMolecule*)g_oaMolecules[m_iSecondMol])->m_sName);
	...
	{
		if (g_iTrajSteps != -1)
			m_iDACFRes = AskUnsignedInteger("    Enter the correlation depth of the DACF in trajectory frames: [%d] ",int(g_iTrajSteps*0.3),int(g_iTrajSteps*0.3));
				else m_iDACFRes = AskUnsignedInteger("    Enter the correlation depth of the DACF in trajectory frames: [5000] ",5000);
	...
	{
		if (g_iTrajSteps != -1)
			m_pNbExchange->m_iTimeDepth = AskUnsignedInteger("    Enter the correlation depth of the neighborhood exchange functions in trajectory frames: [%d] ",int(g_iTrajSteps*0.75),int(g_iTrajSteps*0.75));
				else m_pNbExchange->m_iTimeDepth = AskUnsignedInteger("    Enter the correlation depth of the neighborhood exchange functions in trajectory frames: [5000] ",5000);
	}
	...
	if (g_bDDisp)
	{
		...
		mprintf(YELLOW,"\n*** Dimer Displacement\n\n");
		mprintf("    The \"dimer center\" is the weighted average of 2 atoms from the monomers.\n\n");
...
/*	if (g_bDDisp && g_bDLDF)
...

	{
		...
		m_pCondition->BuildName();


		CxString tstr;

		tstr = (const char*)m_sName;
		tstr += "_";
		tstr += (const char*)m_pCondition->m_sName;

		delete[] m_sName;

		try { m_sName = new char[tstr.GetLength()+1]; } catch(...) { m_sName = NULL; }
		if (m_sName == NULL) NewException((double)(tstr.GetLength()+1)*sizeof(char),__FILE__,__LINE__,__PRETTY_FUNCTION__);
	
		strcpy(m_sName,(const char*)tstr);
		...
		while (AskYesNo("    Add another aggregation function with different criterion values (y/n)? [no] ",false))
		{
			...
			ds->Create(m_pCondition);
			mprintf("\n");
			ds->m_pCondition->Parse_OnlyValues( ds0->m_pCondition );
			ds->Parse();
			ds->BuildName(m_sName);
			m_oaSubDACFs.Add(ds);
			mprintf("\n");
...
/*		if (!g_bInterWarning) {
		...
		if (g_bAdvanced2)
			m_bIntNew = AskYesNo("    Use new ACF-less intermittent mode (y/n)? [yes] ",true);
		else {
			mprintf("\n    Using new ACF-less intermittent mode.\n\n");
			m_bIntNew = true;
		}
	} else {
		m_bNewMode = false;
	...
	if (g_bAdvanced2)
		m_bDebugOut = AskYesNo("    Write debug output for this function (y/n)? [no] ",false);
	else
		m_bDebugOut = false;
...
// New ACF-less intermittent autocorrelation via expansion
