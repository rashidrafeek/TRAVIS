...
#if VOROPP_VERBOSE >=3
				eprintf( "Relocated dangling pointer");
#endif
			}
...
inline bool voronoicell_base::collapse_order2(voronoicell_neighbor &vc) {
	...
	while(mec[2]>0) {
		...
		if(j==k) {
#if VOROPP_VERBOSE >=1
			eprintf( "Order two vertex joins itself");
#endif
			return false;
		}
...
#if VOROPP_VERBOSE >=1
	if(i<1) {
		eprintf( "Zero order vertex formed\n");
		return false;
	}
#endif
	...
	for(us=0;us<nu[up];us++) {
		...
		if(t>g) {
			...
			while (t<rsq) {
				if(++count>=p) {
#if VOROPP_VERBOSE >=1
					eprintf( "Bailed out of convex calculation");
#endif
...
void voronoicell_base::print_edges() {
	...
	for(int i=0;i<p;i++,ptsp+=3) {
		...
		if(ed[i]>=mep[nu[i]]+mec[nu[i]]*((nu[i]<<1)+1)) mprintf( " Memory error");
	}
}
