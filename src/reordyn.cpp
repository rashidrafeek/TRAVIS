...
//_depth:
	...
	mprintf("          Suggested is up to 50 percent of the processed steps.\n\n");
	if (g_iTrajSteps != -1)
		m_iDepth = AskUnsignedInteger("    Enter the correlation depth of the vector ACF (in trajectory frames): [%d] ",int(g_iTrajSteps*0.3),int(g_iTrajSteps*0.3));
			else m_iDepth = AskUnsignedInteger("    Enter the correlation depth of the vector ACF (in trajectory frames): [10000] ",10000);
...
//		if (tf >= 10.0f)
	...
	m_bFitExp = AskYesNo("    Fit poly-exponential functions to reorientation dynamics and integrate over [0,infinity) (y/n)? [yes] ",true);
	if (m_bFitExp) {
		mprintf("\n    TRAVIS will fit functions with different count of exponential terms to your data.\n");
		mprintf("    This enables you to see how many different processes contribute to your reorientation.\n\n");
		if (g_bAdvanced2)
	...
	if (g_bAdvanced2) {
		...
		mprintf("    Note: The question for reorientation spectra has been moved to the advanced mode.\n");
		m_bSpectrum = false;
	}
...
void CReorDyn::ParseSpec()
{
	...
	m_iDepth = AskUnsignedInteger("    Enter the correlation depth of the dipole ACF (in trajectory frames): [%d] ",ti,ti);
...
//	char buf[256];
	...
	if (m_bFitExp) {
		...
		try { m_pRDyn->m_pParameters = new double*[m_iFitDegreeMax+1]; } catch(...) { m_pRDyn->m_pParameters = NULL; }
		if (m_pRDyn->m_pParameters == NULL) NewException((double)(m_iFitDegreeMax+1)*sizeof(double*),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		try { m_pRDyn->m_pFitIntegral = new double[m_iFitDegreeMax+1]; } catch(...) { m_pRDyn->m_pFitIntegral = NULL; }
		if (m_pRDyn->m_pFitIntegral == NULL) NewException((double)(m_iFitDegreeMax+1)*sizeof(double),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		try { m_pRDyn->m_pCorrCoeff = new double[m_iFitDegreeMax+1]; } catch(...) { m_pRDyn->m_pCorrCoeff = NULL; }
		if (m_pRDyn->m_pCorrCoeff == NULL) NewException((double)(m_iFitDegreeMax+1)*sizeof(double),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		try { m_pRDyn->m_pAdditionalSets = new double*[m_iFitDegreeMax+1]; } catch(...) { m_pRDyn->m_pAdditionalSets = NULL; }
		if (m_pRDyn->m_pAdditionalSets == NULL) NewException((double)(m_iFitDegreeMax+1)*sizeof(double*),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		try { m_pRDyn->m_pAdditionalSetLabels = new char*[m_iFitDegreeMax+1]; } catch(...) { m_pRDyn->m_pAdditionalSetLabels = NULL; }
		if (m_pRDyn->m_pAdditionalSetLabels == NULL) NewException((double)(m_iFitDegreeMax+1)*sizeof(char*),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		m_pRDyn->m_iAdditionalSets = m_iFitDegreeMax+1;

		for (z2=0;z2<=m_iFitDegreeMax;z2++) {
			m_pRDyn->m_pAdditionalSets[z2] = NULL;
			m_pRDyn->m_pAdditionalSetLabels[z2] = NULL;
		}

		for (z2=m_iFitDegreeMin;z2<=m_iFitDegreeMax;z2++)
			m_pRDyn->Fit_PolyExp(z2,100);

		if (m_iFitDegreeMin != m_iFitDegreeMax) {
			mprintf(YELLOW,"    Reorientation dynamics fit integral overview:\n");
			mprintf("      (use the row with the highest R value)\n\n");
			mprintf(WHITE,"      Degree  R             Reorientation time / ps\n");
			ti = -1;
			tf = 0;
			for (z2=m_iFitDegreeMin;z2<=m_iFitDegreeMax;z2++) {
				if (m_pRDyn->m_pFitIntegral[z2] >= 0) {
					mprintf("      %d       %10.8f    %-10G\n",z2,m_pRDyn->m_pCorrCoeff[z2],m_pRDyn->m_pFitIntegral[z2]);
					if (m_pRDyn->m_pCorrCoeff[z2] > tf) {
						tf = m_pRDyn->m_pCorrCoeff[z2];
						ti = z2;
					}
				} else
					mprintf("      %d       %10.8f             -         \n",z2,m_pRDyn->m_pCorrCoeff[z2]);
			}
			mprintf("\n");
		} else
			ti = m_iFitDegreeMin;

		mprintf(WHITE,"    Resulting reorientation time:");
		mprintf(" %-10G ps\n\n",m_pRDyn->m_pFitIntegral[ti]);

		if (m_pRDyn->m_pFitIntegral[ti] > g_iTrajSteps*g_fTimestepLength/1000.0) {
			mprintf(RED,"    Warning:");
			mprintf(" The reorientation time is larger than the total trajectory length! The value\n");
			mprintf("             is a pure guess! Use a (much) longer trajectory to obtain reliable reorientation times.\n\n\n");
		} else if (m_pRDyn->m_pFitIntegral[ti] > 0.1*g_iTrajSteps*g_fTimestepLength/1000.0) {
			mprintf(RED,"    Warning:");
			mprintf(" The reorientation time is larger than 10%% of the total trajectory length. The value\n");
			mprintf("             is not reliable. Use a longer trajectory to obtain reliable reorientation times.\n\n\n");
		}
	}
