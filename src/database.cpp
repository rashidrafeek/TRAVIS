...
//	char buf[256];
	...
	for (x=0;x<m_iWidth;x++)
	{
		...
		for (y=0;y<m_iHeight;y++)
		{
			...
			{
				case 1:
					ttw = (unsigned char)((int)log10((double)((signed long)v->m_pValue)))+1;
					if ((signed long)v->m_pValue < 0)
						ttw++;
					break;
					
				case 2:
					ttw = (unsigned char)((int)log10(*((double*)v->m_pValue)))+1;
				...
				case 3:
					ttw = (unsigned char)strlen((char*)v->m_pValue)+2;
					break;
...
//				sprintf(buf,"  [%c%ds-%c%ds]  ",'%',tw,'%',ttw);
			...
			{
				case 1:
					tw = (unsigned char)((int)log10((double)((signed long)v->m_pValue)))+1;
					if ((signed long)v->m_pValue < 0)
						tw++;
					ttw = t[x]-tw;
//					sprintf(buf,"  [%c%ds%cd]  ",'%',ttw,'%');
					...
					break;
				case 2:
					tw = (unsigned char)((int)log10(*((double*)v->m_pValue)))+1;
					if (*((double*)v->m_pValue) < 0)
						tw++;
					tw+=5;
					ttw = t[x]-tw;
//					sprintf(buf,"  [%c%ds%c.4f]  ",'%',ttw,'%');
					...
					break;
				case 3:
					tw = (unsigned char)strlen((char*)v->m_pValue)+2;
					ttw = t[x]-tw;
...
//	char buf[256];
	...
	for (y=0;y<m_iHeight;y++)
	{
		...
		for (z2=0;z2<depth;z2++)
			if ((bitmask & (unsigned long)mypow(2.0,z2)) != 0)
...
void CDatabaseValue::DeleteValue()
{
	if (m_pValue != NULL) {
		switch(m_iType) {
			case 1: break; // int
			case 2: delete (double*)m_pValue; break;
			case 3: delete[] (char*)m_pValue; break;
			case 4: break;
		}
		m_pValue = NULL;
	}
...
CDatabaseNode::~CDatabaseNode()
{
	...
	for (z=0;z<m_oaChildren.GetSize();z++)
		delete (CDatabaseNode*)m_oaChildren[z];

	for (z=0;z<m_oaValues.GetSize();z++)
		delete (CDatabaseValue*)m_oaValues[z];
}
...
void CDatabaseNode::REC_DumpTree(int depth, unsigned long bitmask, bool last)
{
	...
	for (z=0;z<depth-1;z++)
	{
		if ((bitmask & (unsigned long)mypow(2.0,z)) != 0)
//			mprintf("|   ");
			mprintf("%s  ",TreeElement("|"));
				else mprintf("   ");
	}
...
//				mprintf("|\n");
	...
	for (z=0;z<m_oaChildren.GetSize();z++)
	{
		if (z+1 == m_oaChildren.GetSize())
			bitmask -= (unsigned long)mypow(2.0,depth);
...
//	mprintf("CDatabase::AddNode(CxString): \"%s\"\n",(const char*)s);
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddNode(CxString): Invalid format.\n");
		BTOUT;
		return;
	}
...
int CDatabase::GetElementType(CxString s)
{
	...
	if (s[0] != '/')
	{
		mprintf("CDatabase::GetElementType(CxString): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
unsigned long CDatabase::GetInt(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetInt(CxString): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
double CDatabase::GetFloat(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetFloat(CxString): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
int CDatabase::GetArraySize(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArraySize(CxString): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
int CDatabase::GetArrayElementType(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayElementType(CxString,int): Invalid Format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
int CDatabase::GetArrayInt(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayInt(CxString,int): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
double CDatabase::GetArrayFloat(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayFloat(CxString,int): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
bool CDatabase::GetArrayBool(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayBool(CxString,int): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
const char* CDatabase::GetArrayString(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayString(CxString,int): Invalid format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
...
void CDatabase::SetArrayInt(CxString s, int i, int i2)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetArrayInt(CxString,int,int): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetArrayFloat(CxString s, int i, double d)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetArrayFloat(CxString,int,double): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetArrayBool(CxString s, int i, bool b)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetArrayBool(CxString,int,bool): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetArrayString(CxString s, int i, CxString s2)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetArrayString(CxString,int,CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
const char* CDatabase::GetString(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetString(CxString): Invalid format.\n");
		BTOUT;
		return NULL;
	}
	CDatabaseNode *n;
...
bool CDatabase::GetBool(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::GetBool(CxString): Invalid format.\n");
		BTOUT;
		return false;
	}
	CDatabaseNode *n;
...
void CDatabase::AddArray(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddArray(CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
	...
	v->m_sName.Format("%s",(const char*)s.Mid(s.FindLast('/')+1));
	v->m_iType = 5;
...
void CDatabase::AddArrayBool(CxString s, bool b)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddArrayBool(CxString,bool): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddArrayFloat(CxString s, double d)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddArrayFloat(CxString,double): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddArrayInt(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddArrayInt(CxString,int): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddArrayString(CxString s, CxString s2)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddArrayString(CxString,CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddInt(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddInt(CxString,int): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddFloat(CxString s, double d)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddFloat(CxString,double): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddString(CxString s, CxString s2)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddString(CxString,CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::AddBool(CxString s, bool val)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::AddBool(CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetInt(CxString s, int i)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetInt(CxString,int): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetFloat(CxString s, double d)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetFloat(CxString,double): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetString(CxString s, CxString s2)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetString(CxString,CxString): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
void CDatabase::SetBool(CxString s, bool b)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::SetBool(CxString,bool): Invalid format.\n");
		BTOUT;
		return;
	}
	CDatabaseNode *n;
...
bool CDatabase::WriteOutputFile(CxString s)
{
	...
	if (a == NULL)
	{
		eprintf("CDatabase::WriteOutputFile(): Error: Could not open file \"%s\" for writing.\nBe sure to have writing permission for this directory.\n",(const char*)s);
		abort();
	}
...
//	CxString buf, tbuf, t2buf, buf2;
	...
	if (a == NULL)
	{
		eprintf("CDatabase::ParseInputFile(char*): Could not open file \"%s\" for reading.\n",s);
		BTOUT;
		return;
	}
				...
				} else if (value)
				{
					...
					if (tint)
					{
						if ((val=cnode->FindValue(t2buf))!=NULL)
						{
							...
							if (val->m_iType != 1)
							{
								eprintf("CDatabase::ParseInputFile(char*): Error: Redefinition of %s with different type.\n",t2buf);
								mprintf("The configuration file seems to be damaged.\n");
								mprintf("Please delete the file \"%s\" and try it again.\n\n",s);
								abort();
							}
						} else
						{
...
bool CDatabase::ExistNode(CxString s)
{
	...
	if (s[0] != '/')
	{
		eprintf("CDatabase::ExistNode(CxString): Invalid Format.\n");
		BTOUT;
		return 0;
	}
	CDatabaseNode *n;
