...
bool SORT_ElementMass( const CElement* lhs, const CElement* rhs ) {

	return lhs->m_fMass < rhs->m_fMass;
}
...
//	char buf[32768], buf2[32768];
	...
	double tf, tf2, tf3;
	FILE *a;
	bool tb=false, tb2, b, tbnew, rasm;
	int z, z0, z2, z3, z4, z5, z6, z7, timode;
	int ti, ti2, ti3;
	int ty, rty, atom;
	int ty2, rty2, atom2;
	CVirtualAtom *va;
...
//					at->m_pElement->CopyData(el);
				} else
				{
					...
					{
						at->m_pElement->m_fRadius = AskFloat("    Please enter covalent radius in pm: [100] ",100.0);
						at->m_pElement->m_fMass = AskFloat("    Please enter atom mass in u: [10] ",10.0);
					}
				}
			}
_unkdone:
...
_ef:
			if (!tb) {
				...
				mprintf("    such as, e.g., \"CR\" for carbon, which is then erroneously considered as chromium by TRAVIS.\n\n");
			}
	...
	if (((g_iTrajFormat == 5) || (g_iTrajFormat == 7)) && (g_TimeStep.m_pVolumetricData != NULL)) {
		...
		if (g_fCubeXVector[0] < 0) {
			if (!tb) {
				...
				mprintf("    so that the diagonal becomes positive.\n\n");
			}
			g_bFlipCellVectorX = AskYesNo("    Mirror at Y-Z plane so that AX becomes positive (y/n)? [yes] ",true);
		}
		if (g_fCubeYVector[1] < 0) {
			...
			g_bFlipCellVectorY = AskYesNo("    Mirror at X-Z plane so that BY becomes positive (y/n)? [yes] ",true);
		}
		if (g_fCubeZVector[2] < 0) {
			...
			g_bFlipCellVectorZ = AskYesNo("    Mirror at X-Y plane so that CZ becomes positive (y/n)? [yes] ",true);
		}
		if (tb) {
			mprintf("\n");
...
/*	mprintf("    The advanced mode includes many additional options which are quite powerful, yet possibly\n");
	...
	if (g_bXYZComment6Numbers)
	{
		...
		if (AskYesNo("    Are the six numbers in the trajectory's comment line cell geometry data in this format (y/n)? [yes] ",true))
		{
			...
			mprintf("\n    Extracting cell geometry...\n");
			ExtractXYZCellGeometry6(g_TimeStep.m_pComment);
			mprintf("\n");
		} else
	...
	{
		mprintf("    TRAVIS can read cell matrices from the comment line of the XYZ file.\n");
		mprintf("    The format needs to be \"ax ay az bx by bz cx cy cz\" (space separated). Lengths in Angstrom.\n\n");
		mprintf("    The current comment line is \"%s\".\n\n",g_TimeStep.m_pComment);
		if (AskYesNo("    Are the nine numbers in the trajectory's comment line cell geometry data in this format (y/n)? [yes] ",true))
		{
			g_bFoundNonOrtho = true;
			mprintf("\n    Extracting cell geometry...\n");
			ExtractXYZCellGeometry9(g_TimeStep.m_pComment);
			mprintf("\n");
		} else
...
//		}
	...
	if (g_bPeriodic)
	{
		...
		if (g_bNPT)
		{
			if (g_fBoxX != 0)
			{
				mprintf("    Your trajectory file contains cell vector information.\n\n");
			} else
			{
				...
				mprintf("    It should be in one of the following formats:\n\n");
				mprintf("    (*) Orthorhombic cell:     Three numbers (X, Y, Z) per line\n");
				mprintf("    (*) Non-Orthorhombic cell: Nine numbers (A_X, A_Y, A_Z, B_X, B_Y, B_Z, C_X, C_Y, C_Z) per line\n\n");
...
_cellagain:
				...
				g_mBoxFromOrtho(0,1) = AskFloat(    "    Enter Y component of cell vector A in pm: [0.0] ",0.0);
				g_mBoxFromOrtho(0,2) = AskFloat(    "    Enter Z component of cell vector A in pm: [0.0] ",0.0);
				mprintf("\n");
				g_mBoxFromOrtho(1,0) = AskFloat(    "    Enter X component of cell vector B in pm: [0.0] ",0.0);
				...
				g_mBoxFromOrtho(1,2) = AskFloat(    "    Enter Z component of cell vector B in pm: [0.0] ",0.0);
				mprintf("\n");
				g_mBoxFromOrtho(2,0) = AskFloat(    "    Enter X component of cell vector C in pm: [0.0] ",0.0);
				g_mBoxFromOrtho(2,1) = AskFloat(    "    Enter Y component of cell vector C in pm: [0.0] ",0.0);
				...
				if ((fabs(g_mBoxFromOrtho(0,1)) > 1.0e-6) || (fabs(g_mBoxFromOrtho(0,2)) > 1.0e-6) || (fabs(g_mBoxFromOrtho(1,2)) > 1.0e-6)) {
					mprintf("\n");
					eprintf("    Warning: ");
					mprintf("The usual convention is to put cell vector A on the positive X axis, cell vector B in the XY plane\n");
					mprintf("             with positive Y value, and cell vector C with positive Z value. You did not keep to this convention.\n");
					mprintf("             Several functions may not work properly. You should rotate your cell to keep this conventions.\n\n");
					(void)AskYesNo("    Acknowledged: [Ok] ",true);
				}
				...
				mprintf("\n    Convention: \"Vector A\" is located on the X axis, \"vector B\" in the XY plane with positive Y value.\n\n");
...
_celldefined:
	...
	std::set<CElement*, bool(*)(const CElement* lhs, const CElement* rhs)> els( &SORT_ElementMass );
	std::set<CElement*, bool(*)(const CElement* lhs, const CElement* rhs)>::iterator elsit;

	for (z=0;z<g_oaAtoms.GetSize()-1;z++) {
		if (((CAtom*)g_oaAtoms[z])->m_pMergedTo != NULL)
			continue;
		els.insert( ((CAtom*)g_oaAtoms[z])->m_pElement );
	}

	mprintf("      Element | Ord. Number |  Mass / u | Cov. Radius / pm | VdW Radius / pm | Color\n");
	mprintf("    ----------|-------------|-----------|------------------|-----------------|----------\n");

	for (elsit=els.begin();elsit!=els.end();++elsit) {
		tel = *elsit;
		mprintf(
			"      %-7s | %11d | %9.5f | %16.2f | %15.2f | #%02X%02X%02X\n",
			tel->m_sLabel,
			tel->m_iOrd,
			tel->m_fMass,
			tel->m_fRadius,
...
_matrixagain:
	...
	for (z=0;z<g_oaMolecules.GetSize();z++)
	{
		...
		//mprintf("      Max. diameter:  %.2f pm\n", m->m_fMaxDiameter );
		if (g_bBoxNonOrtho)
			mprintf("      Concentration:  %9.6f mol/liter\n", m->m_laSingleMolIndex.GetSize()/CONST_AVOGADRO/(g_fBoxVolume*1000000.0*1.0e-33) );
		else
			mprintf("      Concentration:  %9.6f mol/liter\n", m->m_laSingleMolIndex.GetSize()/CONST_AVOGADRO/(g_fBoxX*g_fBoxY*g_fBoxZ*1.0e-33) );
	...
	{
		if (g_bAdvanced1) {
			if (AskYesNo("    Render structural formulas only for first molecule of each kind (y) or for all molecules (n)? [yes] ",true))
				rasm = false;
			else
				rasm = true;
...
_modagain:
		...
		mprintf("    4.) Split molecule type into two types\n");
		mprintf("    5.) Add bonds / add solvation shell to molecule\n");

		mprintf("\n");
		switch(AskRangeInteger("    Please select: [done] ",1,5,0))
		{
			case 1: // Change radii
				...
				mprintf("\n");
_arnew:
				AskString("    The radius of which element do you want to change (enter element label)? [done] ",&buf,"");
				if (strlen(buf) == 0)
					goto _ardone;
				for (z=0;z<g_oaAtoms.GetSize()-1;z++)
				{
				...
				eprintf("    Element \"%s\" is not in the system.\n\n",(const char*)buf);
				goto _arnew;
_ardone:
				mprintf("\n");
				break;
...
_breakmol:			ti = AskUnsignedInteger_ND("    Break bonds in which molecule (1-%d)? ",g_oaMolecules.GetSize())-1;
				...
				{
_breaka1:			AskString_ND("    Enter 1st atom of the bond to break (e.g. C1): ",&buf);
					if (!ParseAtom(buf,ti,ty,rty,atom))
						goto _breaka1;
_breaka2:			AskString_ND("    Enter 2nd atom of the bond to break (e.g. C2): ",&buf);
...
/*********************************************************************************************************************/
			...
			case 4:
				tb = true;
				mprintf("\n    The system contains the following molecule types:\n");
				for (z=0;z<g_oaMolecules.GetSize();z++) {
					m = (CMolecule*)g_oaMolecules[z];
					mprintf("      - Molecule %d: %s ",z+1,m->m_sName);
					mprintf("(%d piece%s, %.2f g/mol)\n",m->m_laSingleMolIndex.GetSize(),(m->m_laSingleMolIndex.GetSize()>1)?"s":"",m->m_fMass);
				}
				mprintf("\n");
				if (g_oaMolecules.GetSize() > 1)
					ti = AskRangeInteger_ND("    Which molecule type do you want to split (1-%d)? ",1,g_oaMolecules.GetSize(),g_oaMolecules.GetSize()) - 1;
				else {
					mprintf("    Only one molecule type, splitting this one.\n\n");
					ti = 0;
				}
				m = (CMolecule*)g_oaMolecules[ti];
_splitagain:
				AskString_ND("    Enter list of molecules from this type to split off (range 1-%d, e.g. \"1,5-11,17\"): ",&buf,m->m_laSingleMolIndex.GetSize());
				tia.clear();
				...
				for (z=0;z<(int)tia.size();z++) {
					if ((tia[z] < 1) || (tia[z] > m->m_laSingleMolIndex.GetSize())) {
						eprintf("Error: Molecule ID out of range: %d.\n",tia[z]);
						goto _splitagain;
					}
					for (z2=0;z2<z;z2++) {
						if (tia[z] == tia[z2]) {
							eprintf("Error: Molecule ID appears twice in list: %d.\n",tia[z]);
							goto _splitagain;
						}
					}
				}
				g_iaaMoleculeTypeSplit.push_back( std::vector<int>() );
				for (z=0;z<(int)tia.size();z++)
					g_iaaMoleculeTypeSplit.back().push_back( m->m_laSingleMolIndex[ tia[z]-1 ] );
				mprintf("\n    Will split off %lu molecules from molecule type %d.\n",(unsigned long)tia.size(),ti+1);
				break;

			case 5: // Add bonds
				tb = true;
				mprintf("\n");
				if (AskYesNo("    Add solvation shell to molecule (y) or add specific bonds (n)? [yes] ",true)) {

					ti = AskRangeInteger_ND("    Add solvation shell around which molecule type (1-%d)? ",1,g_oaMolecules.GetSize(),g_oaMolecules.GetSize()) - 1;

					ti2  = AskRangeInteger_ND("    Which is the solvent molecule type (1-%d)? ",1,g_oaMolecules.GetSize(),g_oaMolecules.GetSize()) - 1;

					if (ti == ti2) {
						eprintf("Error: The molecule types need to differ.\n");
						abort();
					}

					tf = AskFloat("    Enter the bond distance threshold between atoms from solute and solvent (in pm): [300.0] ",300.0);

					if (((CMolecule*)g_oaMolecules[ti])->m_laSingleMolIndex.GetSize() > 1) {
						eprintf("Not implemented for more than one solute molecule.\n");
						abort();
					}
					...
					m2 = (CMolecule*)g_oaMolecules[ti2];

					ti3 = 0;

					for (z=0;z<m2->m_laSingleMolIndex.GetSize();z++) {

						sm2 = (CSingleMolecule*)g_oaSingleMolecules[m2->m_laSingleMolIndex[z]];
						...
						for (z2=0;z2<sm->m_oaAtomOffset.GetSize()-1;z2++) { // "-1" to skip virtual atoms
							...
							for (z3=0;z3<((CxIntArray*)sm->m_oaAtomOffset[z2])->GetSize();z3++) {
								...
								for (z4=0;z4<sm2->m_oaAtomOffset.GetSize()-1;z4++) { // "-1" to skip virtual atoms

									for (z5=0;z5<((CxIntArray*)sm2->m_oaAtomOffset[z4])->GetSize();z5++) {

										tf2 = FoldedLength(
											g_TimeStep.m_vaCoords[ ((CxIntArray*)sm->m_oaAtomOffset[z2])->GetAt(z3) ] -
											g_TimeStep.m_vaCoords[ ((CxIntArray*)sm2->m_oaAtomOffset[z4])->GetAt(z5) ]
										);
										...
										if (tf2 <= tf) {
								//			mprintf("@ z=%d z2=%d z3=%d z4=%d z5=%d, tf2=%f o1=%d o2=%d\n",z,z2,z3,z4,z5,tf2,
								//				((CxIntArray*)sm->m_oaAtomOffset[z2])->GetAt(z3),
								//				((CxIntArray*)sm2->m_oaAtomOffset[z4])->GetAt(z5)
								//			);
											g_laBondAddList.Add( ((CxIntArray*)sm->m_oaAtomOffset[z2])->GetAt(z3) );
											g_laBondAddList.Add( ((CxIntArray*)sm2->m_oaAtomOffset[z4])->GetAt(z5) );
											ti3++;
											goto _abdone;
										}
									}
				...
				//		mprintf("( z=%d, smallest distance %f Angstrom )\n",z,tf3);
_abdone:;
					}

					mprintf("    Added %d solvent molecules to solute molecule.\n\n",ti3);
...
_fin:
	...
	for (z=0;z<g_oaMolecules.GetSize();z++) {
		...
		for (z2=0;z2<m->m_laSingleMolIndex.GetSize();z2++) {
			...
			for (z3=0;z3<sm->m_oaMolAtoms.GetSize();z3++) {
				for (z4=0;z4<z3;z4++) {
					tf = (g_TimeStep.m_vaCoords[ ((CMolAtom*)sm->m_oaMolAtoms[z3])->m_iOffset ] -
						g_TimeStep.m_vaCoords[ ((CMolAtom*)sm->m_oaMolAtoms[z4])->m_iOffset ]).GetLength();
					if (tf > m->m_fMaxDiameter)
						m->m_fMaxDiameter = tf;
				}
			}
	...
	mprintf("\n");
	mprintf(WHITE,"    Found the following maximal molecular diameters:\n");
	for (z=0;z<g_oaMolecules.GetSize();z++) {
		m = (CMolecule*)g_oaMolecules[z];
		mprintf("\n");
		mprintf(YELLOW,"      (*)");
		mprintf(" Molecule %d (%s):  ",z+1,m->m_sName);
		for (z2=0;z2<ti-(int)strlen(m->m_sName);z2++)
			mprintf(" ");
		if (m->m_bPolymer) {
			mprintf("(is a polymer)\n");
			continue;
		}
		if (m->m_iAtomGesNoVirt == 1) {
			mprintf("(only one atom)\n");
			continue;
		}
		mprintf("%8.2f pm\n",m->m_fMaxDiameter);
		if (g_bPeriodic) {
			if (g_fBoxMinDiam - m->m_fMaxDiameter < 300.0) {

				eprintf("          Warning:");
				mprintf(" The molecule is almost as large as the box; there are only %.2f pm of separation!\n",g_fBoxMinDiam - m->m_fMaxDiameter);
			}
		}
	}
	mprintf("\n");
...
//			strcat(buf,")? ");
			...
			mprintf("\n");
				...
				timode = (unsigned char)AskRangeInteger("    Define v.a. as center (0) or through distance/angle/dihedral (1)? [center] ",0,1,0);
	...
	_vaatoms:
				...
				va->m_faWeight.SetSize(va->m_oCenterAtoms.m_iAtomGes);
	_vaweight:
				mprintf("    How shall these atoms be weighted (RETURN=equal, *=mass, #=manually)? [equal] ");
				inpprintf("! How shall these atoms be weighted (RETURN=equal, *=mass, #=manually)? [equal]\n");
				myget(&buf);
				if (strlen(buf)==0)
				{
					for (z=0;z<va->m_oCenterAtoms.m_iAtomGes;z++)
...
_nopseudomol:
	...
	mprintf("      starting in center of mass (#2). 1 Debye corresponds to 100 pm of vector length.\n");
...
_fncinput:
	...
	if (g_bVDF || g_bRaman || g_bCombined || g_bBondACF || g_bVHDF || g_bACF || g_bUseVelocities || g_bUseForces || g_bMSD || g_bAggregation || g_bDLDisp || g_bDLDF || g_bDACF || g_bRDyn || g_bIRSpec || g_bTimeDiff || g_bDeriv || g_bNormalCoordinate || g_bPower || g_bVACFNew || g_bIR || g_bSetUpPolarizabilityCalc || g_bFDF || g_bVCD || g_bMagneticDipoleRestart || g_bTDDF)
	{
...
_entertype:
	...
	if (g_bTegri && (g_pTetraPak == NULL))
	{
		...
		if (g_bMinimizeChargeVar2) {
			g_pChargeVar2 = new CChargeVar2();
			g_pChargeVar2->Parse();
			g_bAbortAnalysis = false;
			g_pChargeVar2->PerformAnalysis();
			return true;
		}
	#endif
...
/*	if (g_bSFac)
	...
	if (g_bAggrTopo) {
		...
		try { g_pAggrTopoEngine = new CAggrTopoEngine(); } catch(...) { g_pAggrTopoEngine = NULL; }
		if (g_pAggrTopoEngine == NULL) NewException((double)sizeof(CAggrTopoEngine),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		if (!g_pAggrTopoEngine->Parse())
			return false;
	}

	if (g_bContactMatrix) {

		try { g_pContactMatrix = new CContactMatrix(); } catch(...) { g_pContactMatrix = NULL; }
		if (g_pContactMatrix == NULL) NewException((double)sizeof(CContactMatrix),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		if (!g_pContactMatrix->Parse())
			return false;
	}

	if (g_bGeoDens) {

		try { g_pGeoDensEngine = new CGeoDensEngine(); } catch(...) { g_pGeoDensEngine = NULL; }
		if (g_pGeoDensEngine == NULL) NewException((double)sizeof(CGeoDensEngine),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		if (!g_pGeoDensEngine->Parse())
			return false;
	}

	if (g_bTDDF) {

		try { g_pTDDFEngine = new CTDDFEngine(); } catch(...) { g_pTDDFEngine = NULL; }
		if (g_pTDDFEngine == NULL) NewException((double)sizeof(CTDDFEngine),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		if (!g_pTDDFEngine->Parse())
			return false;
	}
...
//		mprintf(YELLOW,"\n<<< End of Select Autocorrelation Functions <<<\n\n");
	...
	if (g_bBondACF)
	{
		...
		if (g_iTrajSteps != -1)
			g_iBondACFDepth = AskUnsignedInteger("    Enter bond ACF correlation depth in trajectory frames: [%d] ",g_iTrajSteps/2,g_iTrajSteps/2);
				else g_iBondACFDepth = AskUnsignedInteger("    Enter bond ACF correlation depth in trajectory frames: [%d] ",4096,4096);
	...
	if (g_bSaveRefEnv || g_bCutCluster)
	{
		...
		if (g_bSaveRefEnv)
		{
			if (AskYesNo("    Create a temporal development overlay (TDO) plot (y/n)? [no] ",false))
			{
				...
				g_fTDOBleaching = AskRangeFloat("    Enter the TDO bleaching degree (0-1): [0.8] ",0.0,1.0,0.8);
			} else g_bTDO = false;
		}
	}
		...
		if (o->m_bOthers)
		{
			if (g_bCDF)
			{
				...
				if (g_bAdvanced2)
					...
					mprintf("    Switch on the \"advanced mode\" to perform a three-body analysis.\n");
			...
			{
				mprintf(WHITE,"    You have to select two observed molecules (OMs):\n");
			...
			if (!o->m_bSecondShowMol)
			{
				...
				if (g_bAdvanced2)
				{
					if (AskYesNo("    Create a temporal difference plot for this observation (y/n)? [no] ",false))
					{
						o->m_bTimeDiff = true;
						g_bTimeDiff = true;
						o->m_iTimeDiffDepth = AskUnsignedInteger("    Enter temporal depth of this plot in trajectory frames: [1000] ",1000);
...
//		g_bTrajAtomwise = (AskRangeInteger("\n    Save trajectory ordered molecule-wise (0) or atom-wise (1)? [0] ",0,1,0)!=0);
		...
		if (g_bAdvanced2) {
			...
			g_bSaveTrajPattern = AskYesNo("    Select frames to store according to a pattern (y/n)? [no] ",false);
			if (g_bSaveTrajPattern) {
				mprintf("\n");
				mprintf("    This feature allows to store non-uniform sequences of frames, such as\n");
				mprintf("    \"save 3 frames, skip 5 frames, repeat this protocol until end of trajectroy\".\n");
				mprintf("    This can be useful for computing magnetic moments for VCD and ROA spectra.\n\n");
				mprintf(YELLOW,"    Notes:\n\n");
				mprintf(WHITE,"      (*) ");
				mprintf("You need to enter a string which only consists of \"0\" and \"1\" characters.\n");
				mprintf("          \"0\" means \"skip\", \"1\" means \"store\". The total number\n");
				mprintf("          of characters determines the periodicity of the pattern.\n\n");
				mprintf(WHITE,"      (*) ");
				mprintf("The pattern only acts on the frames which are active according to the rules below\n");
				mprintf("          (e.g. \"process only every n-th frame\").\n\n");
_patternagain:
				AskString("    Enter the pattern: [11100000] ",&buf,"11100000");
				ti = 0;
				for (z=0;z<buf.GetLength();z++) {
					if ((buf[z] != '0') && (buf[z] != '1')) {
						eprintf("Error: Invalid character \"%c\". Only \"0\" and \"1\" are allowed.\n",buf[z]);
						goto _patternagain;
					}
					if (buf[z] == '1')
						ti++;
				}
				g_sSaveTrajPattern = buf;
				mprintf("\n");
				mprintf("    Will store %d out of %d frames according to pattern.\n",ti,buf.GetLength());
				mprintf("\n");
			}
		} else {
...
//		mprintf("\n    Not wrapping molecules into the box.\n");
	...
	if (g_bMSD || g_bPairMSD || (g_bSaveJustTraj && !g_bSaveCoordsUnchanged) || g_bACF || (g_bIRSpec && g_bDipoleRefFixed) || g_bVHDF || g_bPDF || g_bFixedPlProj)
	{
	...
	if (g_bUseVelocities && !g_bReadVelocity && (g_fTimestepLength > 1.0) && !g_bROA)
	{
		mprintf(RED,"    Warning: ");
	...
	if ((g_TimeStep.GetCommentNumberCount() > 0) && (g_iTrajFormat != 9))
	{
...
//					strcat(buf," [1] ");
	...
	if (g_bAdvanced2)
	{
		if (AskYesNo("    Perform a multi-interval analysis (y/n)? [no] ",false))
		{
			...
			mprintf("             automated by a small shell script.\n\n");
			if (!AskYesNo("    Acknowledged (y/n) [yes] ",true))
				return false;
			mprintf("\n");
