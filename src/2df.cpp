...
//	char buf1[32768], buf2[32768], buf[32768];
	CxString buf1, buf2, bufvec, buf, out, out2, outvec;
	int minorx, majorx, minory, majory, ti;
	double bx, by;
...
//	sprintf(buf1,"%s%s%s.gp.csv",prefix,name,suffix);
	...
	if (m_bVectorField) {
		...
		bufvec.sprintf("%s%s%s_vector.gp.csv",prefix,name,suffix);
		a = OpenFileWrite(bufvec,true,&outvec);

		for (z2=0;z2<m_iVectorRes[1];z2++) {
			by = m_fMinVal[1]+(m_fMaxVal[1]-m_fMinVal[1])*(((double)z2+0.5)/m_iVectorRes[1]);
			for (z=0;z<m_iVectorRes[0];z++) {
				bx = m_fMinVal[0]+(m_fMaxVal[0]-m_fMinVal[0])*(((double)z+0.5)/m_iVectorRes[0]);
		//		mprintf("z=%d, min=%f, max=%f, res=%d, f=%f\n",z,m_fMinVal[0],m_fMaxVal[0],m_iVectorRes[0],bx);
				mfprintf(
					a,
					"%.10G;  %.10G;  %.10G;  %.10G\n",
					bx-m_faVectorData[(z2*m_iVectorRes[0]+z)*2]/2.0,
					by-m_faVectorData[(z2*m_iVectorRes[0]+z)*2+1]/2.0,
					m_faVectorData[(z2*m_iVectorRes[0]+z)*2],
					m_faVectorData[(z2*m_iVectorRes[0]+z)*2+1]
				);
			}
		}
...
//	sprintf(buf2,"%s%s%s.gp",prefix,name,suffix);
	...
	if (manrange) {
		mfprintf(b,"s_zauto = 0\n");
		mfprintf(b,"# Minimum Z\n");
		mfprintf(b,"s_zmin = %f\n", m_fCustomValueMin );
		mfprintf(b,"# Maximum Z\n");
		mfprintf(b,"s_zmax = %f\n", m_fCustomValueMax );
		mfprintf(b,"# Major Z tics distance\n");
		mfprintf(b,"s_ztics = %f\n", (m_fCustomValueMax-m_fCustomValueMin)/5.0 );
	...
	if (m_bVectorField) {
		mfprintf(b,"# Length factor for vectors\n");
		mfprintf(b,"s_vecfac = %.3f\n",(m_fMaxVal[0]-m_fMinVal[0])/20.0);
		mfprintf(b,"\n");
		mfprintf(b,"# Length exponent for vectors\n");
		mfprintf(b,"s_vecexp = 1.0\n");
		mfprintf(b,"\n");
		mfprintf(b,"# Head Size for vectors\n");
		mfprintf(b,"s_vechead = %.2f\n",m_fVectorHeadSize);
		mfprintf(b,"\n");
	}
	...
	if (m_oaLines.GetSize() > 0) {
		mfprintf(b,"# Line definitions\n");
		for (z=0;z<m_oaLines.GetSize();z++)
			mfprintf(
				b,
				"set arrow %d from %f,%f to %f,%f nohead back linewidth %f linecolor rgb '#%02X%02X%02X'\n",
				++ti,
				((CMathematicaLine*)m_oaLines[z])->m_fPosX1,
				((CMathematicaLine*)m_oaLines[z])->m_fPosY1,
				((CMathematicaLine*)m_oaLines[z])->m_fPosX2,
				((CMathematicaLine*)m_oaLines[z])->m_fPosY2,
				((CMathematicaLine*)m_oaLines[z])->m_fWidth,
				(int)(((CMathematicaLine*)m_oaLines[z])->m_fColorR*255.0),
				(int)(((CMathematicaLine*)m_oaLines[z])->m_fColorG*255.0),
				(int)(((CMathematicaLine*)m_oaLines[z])->m_fColorB*255.0)
			);
		mfprintf(b,"\n");
		fflush(b);
	}
	...
	if (m_oaCircles.GetSize() > 0) {
		mfprintf(b,"# Circle definitions\n");
		for (z=0;z<m_oaCircles.GetSize();z++)
			mfprintf(
				b,
				"set object %d circle at %f,%f size first %f fillstyle solid 1.0 border -1 linewidth 0.5 fillcolor rgb '#%02X%02X%02X'\n",
				++ti,
				((CMathematicaCircle*)m_oaCircles[z])->m_fPosX,
				((CMathematicaCircle*)m_oaCircles[z])->m_fPosY,
				((CMathematicaCircle*)m_oaCircles[z])->m_fRadius,
				(int)(((CMathematicaCircle*)m_oaCircles[z])->m_fColorR*255.0),
				(int)(((CMathematicaCircle*)m_oaCircles[z])->m_fColorG*255.0),
				(int)(((CMathematicaCircle*)m_oaCircles[z])->m_fColorB*255.0)
			);
		mfprintf(b,"\n");
	}
	...
	mfprintf(b,"\n");
	mfprintf(b,"f_signpow(x,p) = sgn(x)*(abs(x)**p)\n");
	mfprintf(b,"\n");
...
//	mfprintf(b,"set size 1.15,1.15\n");
	mfprintf(b,"set origin 0.0, 0.0\n");
	mfprintf(b,"set size 1.1, 1.1\n");
	mfprintf(b,"\n");
...
//	mfprintf(b,"splot \"test5.dat\" matrix using ($2*1.5+50.75):($1*5.0+2.5):(f_plot($3))\n");
	...
	if (m_bVectorField)
		mfprintf(b,"splot \"%s\" using ($1-f_signpow($3,s_vecexp)*s_vecfac/2.0):($2-f_signpow($4,s_vecexp)*s_vecfac/2.0):(0):(f_signpow($3,s_vecexp)*s_vecfac):(f_signpow($4,s_vecexp)*s_vecfac):(1) with vectors head size s_vechead,20,60 filled linewidth 0.5 linecolor rgbcolor \"black\"\n\n",(const char*)bufvec);
...
//	strcat(buf,suffix);
	...
	mfprintf(a,"   RowBox[{\n");
	mfprintf(a,"    RowBox[{\n");
	mfprintf(a,"    \"Choose\", \" \", \"the\", \" \", \"smoothing\", \" \", \"degree\", \" \", \"for\", \" \", \n");
	mfprintf(a,"     \"your\", \" \", \n");
	...
	if (m_oaLines.GetSize() > 0) {
		...
		for (z=0;z<m_oaLines.GetSize();z++) {
			...
			mfprintf(a,"       RowBox[{\"{\", \n");
			mfprintf(a,"        RowBox[{\n");
			mfprintf(a,"         RowBox[{\"AbsoluteThickness\", \"[\", \"%f\", \"]\"}], \",\", \n",((CMathematicaLine*)m_oaLines[z])->m_fWidth*3.0);
			mfprintf(a,"         RowBox[{\"RGBColor\", \"[\", \n");
			mfprintf(a,"          RowBox[{\"%f\", \",\", \"%f\", \",\", \"%f\"}], \"]\"}], \",\", \n",((CMathematicaLine*)m_oaLines[z])->m_fColorR,((CMathematicaLine*)m_oaLines[z])->m_fColorG,((CMathematicaLine*)m_oaLines[z])->m_fColorB);
			mfprintf(a,"         RowBox[{\"Dashing\", \"[\", \"1000\", \"]\"}], \",\", \n");
			mfprintf(a,"         RowBox[{\"Line\", \"[\", \n");
			mfprintf(a,"          RowBox[{\"{\", \n");
			mfprintf(a,"           RowBox[{\n");
			mfprintf(a,"            RowBox[{\"{\", \n");
			mfprintf(a,"             RowBox[{\"%f\", \",\", \"%f\"}], \"}\"}], \",\", \n",((CMathematicaLine*)m_oaLines[z])->m_fPosX1,((CMathematicaLine*)m_oaLines[z])->m_fPosY1);
			mfprintf(a,"            RowBox[{\"{\", \n");
			mfprintf(a,"             RowBox[{\"%f\", \",\", \"%f\"}], \"}\"}]}], \"}\"}], \"]\"}]}],\n",((CMathematicaLine*)m_oaLines[z])->m_fPosX2,((CMathematicaLine*)m_oaLines[z])->m_fPosY2);
			mfprintf(a,"         \"}\"}], \"]\"}]}], \"]\"}], \";\"}]\n");

			if (z < m_oaLines.GetSize()-1)
				mfprintf(a,", \"\\[IndentingNewLine]\", ");
			mfprintf(a,"\n");
		}
		mfprintf(a,"}]\n");
...
void C2DF::AddLine(double x1, double y1, double x2, double y2, double width, double cr, double cg, double cb)
{
	CMathematicaLine *c;

	try { c = new CMathematicaLine(); } catch(...) { c = NULL; }
	if (c == NULL) NewException((double)sizeof(CMathematicaLine),__FILE__,__LINE__,__PRETTY_FUNCTION__);
