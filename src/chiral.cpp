...
//		char buf[BUF_SIZE];
	...
	for(i = 0; i < m_iShowMolCount; i++) {
		...
		do {
			...
			li = 0;
			for(j = 0; j < 4; j++)
				if((a->GetAt(j) == -1) && (((CMolAtom *)ma->m_oaBonds[j])->m_liAtomCode > li))
					li = ((CMolAtom *)ma->m_oaBonds[j])->m_liAtomCode;
			...
			while(true) {
				...
				for(j = 0; j < 4; j++) {
					...
					if((a->GetAt(j) == -1) && (((CMolAtom *)ma->m_oaBonds[j])->m_liAtomCode == li) && (((CMolAtom *)ma->m_oaBonds[j])->m_iNumber < minNum)) {
...
/*#ifdef TARGET_LINUX
			...
			if(((CMolAtom *)((CMolAtom *)_molAtoms[i])->m_oaBonds[((CxIntArray *)_orderArrays[i])->GetPosition(a1-1)])->m_liAtomCode != ((CMolAtom *)((CMolAtom *)_molAtoms[i])->m_oaBonds[((CxIntArray *)_orderArrays[i])->GetPosition(a2-1)])->m_liAtomCode)
